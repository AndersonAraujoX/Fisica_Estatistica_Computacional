program QC
  ...
  do while(b<=be)
    ...
    r=10+r0
  	y=r+l/2!soltar a partiular na parte superior
    x=l*rand()
    !do para fazer contagem de passos do bebados nos 4 sentidos
      ...
      !write(*,*)x,y
      if(x>0 .and. y>0)then
        if ((frac(x+1,y)+frac(x-1,y)+frac(x,y+1)+frac(x,y-1))>=1)then
          frac(x,y)=1!quando a particula encontra o agregado
          b=b+1
          exit
        endif
      endif
      if(abs(y-l/2)>1.5*r .or. x<0 .or. y<0)then!verificar se a particula foi para longe 
    ...
    enddo
    if(x>0 .and. y>0)then
      if(abs(y-l/2)>r0 .and. frac(x,y)==1) then!verificar se o novo agregado e a particula mais longe
        r0=abs(y-l/2)
      endif
    endif
    write(2,*)r,b
