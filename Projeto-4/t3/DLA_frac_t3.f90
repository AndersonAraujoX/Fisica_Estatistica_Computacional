program QC
  ...
  do while(b<=be)
    ...
    teta=rand()
    phi=rand()
  	x=r*cos(2*pi*teta)*sin(2*pi*rand())+l/2
    y=r*sin(2*pi*teta)*sin(2*pi*rand())+l/2
    z=r*cos(2*pi*rand())+l/2
    !do para fazer contagem de passos do bebados nos 4 sentidos
      ...
      if(x>0 .and. y>0 .and. z>0)then
        if ((frac(x+1,y,z)+frac(x-1,y,z)+frac(x,y+1,z)+frac(x,y-1,z)+frac(x,y,z+1)+frac(x,y,z-1))>=1 )then
  ...
  enddo
  !do j=1,l
    !  write(1,*)(frac(l/2,j,a),a=1,l)
   ! enddo
  !do i=1,l
   ! do j=1,l
   !   write(10+i,*)(frac(i,j,a),a=1,l)
   ! enddo
  !enddo
end program
